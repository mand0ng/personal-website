...
async def get_product(product_div):
    ...
    try:
        ...
        product_price = None
    product_url = "http://amazon.ca"+"/".join((await url_element.get_attribute('href')).split("/")[:4]) if url_element else None
    # stock = stock_element[0] if len(stock_element) > 0 else None

    return {"img": image_url, "name": product_name, "price": product_price, "url": product_url, "product_rating": 5}

async def get_todays_deal_product(product_div):
    # Query for all elements at once
    image_element_future = product_div.query_selector('img._deals-shoveler-v2_style_dealImage__3nlqg')
    name_element_future = product_div.query_selector('span.a-truncate-cut')
    promo_off_elements_future = product_div.query_selector_all('div._deals-shoveler-v2_style_badgeLabel__rzdVa span')
    url_element_future = product_div.query_selector('a.a-link-normal.a-text-normal')

    # Await all queries at once
    image_element, name_element, promo_off_elements, url_element = await gather(
        image_element_future,
        name_element_future,
        promo_off_elements_future,
        url_element_future,
        # get_stock(product_div)
    )
    ...
    image_url = await image_element.get_attribute('src') if image_element else None
    product_name = await name_element.inner_text() if name_element else None
    price_promo = ' '.join([await span_element.inner_text() for span_element in promo_off_elements]) if promo_off_elements else None
    product_url = "http://amazon.ca" + await url_element.get_attribute('href') if url_element else None

    return {"img": image_url, "name": product_name, "promo": price_promo, "url": product_url, "product_rating": 5}
